...
# for LONG_FILE in `ls -1 ${START_DIR}/*06-02-2020*.csv`
do
	...
	psql -d barchart << EOF
		...
		\copy stg2 from $LONG_FILE DELIMITER ',' CSV HEADER

		-- INSERT INTO ATOMIC TABLE.
		INSERT INTO barchart_data
		SELECT symbol, '$CURR_DATE', '0' AS last_price,
		CASE 
		...
		END chg_12m
		FROM stg2
		EXCEPT SELECT symbol, data_date, last_price, weighted_alpha, perc_change_daily, perc_chg_week, perc_chg_1mth, perc_chg_3mth, perc_chg_6mth, perc_chg_year
		FROM barchart_data;
