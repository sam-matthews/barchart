CREATE OR REPLACE FUNCTION ret_6mth(
    dur_type IN VARCHAR, 
...
BEGIN
  ...
  FOR ref IN SELECT * FROM lkp_dates WHERE prev_date BETWEEN date_start AND date_stop ORDER BY 1 LOOP
    ...
    IF (dur_type = 'month' AND start_of_month = 'true') THEN
      ...
      ORDER BY perc_chg_6mth::decimal DESC LIMIT stocks_to_choose;
  ...
  -- raise notice 'Loading data into: %', ref.data_date;
    ...
    INSERT INTO summary  
    SELECT b.symbol, b.data_date, '6-mth', dur_type, stocks_to_choose, b.perc_change_daily 
  ...
  PERFORM FROM calculate_running_total('6-mth', dur_type, stocks_to_choose);
