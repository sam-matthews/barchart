CREATE OR REPLACE FUNCTION ret_year(
    dur_type IN VARCHAR, 
...
BEGIN
  ...
  FOR ref IN SELECT * FROM lkp_dates WHERE prev_date BETWEEN date_start AND date_stop ORDER BY 1 LOOP
    ...
    IF ((EXTRACT(MONTH FROM ref.data_date) IN (1,4,7,10)) AND start_of_month = 'true')
    THEN
      start_of_quarter := 'true';
    ELSE
      start_of_quarter := 'false';
    END IF;
    ...
    -- IF Quarter Analysis is being run.
    IF (dur_type = 'quarter' AND start_of_quarter)
    THEN

      raise notice 'Running Quarter analysis';
      ...
      ORDER BY perc_chg_year::decimal DESC LIMIT stocks_to_choose;

    END IF;

    -- IF 2Month Analysis is being run.
    ...
    -- IF Month Analysis is being run.
  ...
  -- raise notice 'Loading data into: %', ref.data_date;
    ...
    INSERT INTO summary  
    SELECT b.symbol, b.data_date, 'year', dur_type, stocks_to_choose, b.perc_change_daily 
  ...
  PERFORM FROM calculate_running_total('year', dur_type, stocks_to_choose);
